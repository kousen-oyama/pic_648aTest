...
#pragma config LVP = ON        //低電圧で書き込みするやつ。私にはいらない。
#pragma config CPD = OFF       //コードの保護とかそんなの。しらん。 
#pragma config CP = OFF        //上と同じようなの。いらね


#include <xc.h>
#define _XTAL_FREQ 4000000 //デフォルトでは周波数が4MHz
...
int main(void)
{
    ...
    TRISB = 0x00;       //こちらも入出力。0x00=0b00000000 0=出力 1=入力
    while (1){
      //PORTB = 0b00000010;        //このように、全体を一括で管理可能。
        RB1 = 1;                   //上と同じことをしている
        __delay_ms(200);           //（注）先頭の__は、アンダーバー2つ。()内に変数をつかうことはできない。
      //PORTB = 0b00000000;
        RB1 = 0;
        __delay_ms(200);
    }
...
//上にチェックを入れ、電圧を5Vあたり(私は4.75で利用中)にすること。
